...
public class XNRTSuggester extends XLookup {
     ...
     * suggestions.
     */
    public static final int DEFAULT_MAX_EDITS = 1; //DEBUG change to 0
    ...
    public XNRTSuggester(Analyzer indexAnalyzer, Automaton queryPrefix, Analyzer queryAnalyzer, int options, int maxSurfaceFormsPerAnalyzedForm, int maxGraphExpansions,
        ...
        this(indexAnalyzer, queryPrefix, queryAnalyzer, options, maxSurfaceFormsPerAnalyzedForm, maxGraphExpansions, preservePositionIncrements, fst,
                hasPayloads, maxAnalyzedPathsForOneInput, sepLabel, payloadSep, endByte, holeCharacter,
                DEFAULT_MAX_EDITS, DEFAULT_TRANSPOSITIONS, DEFAULT_NON_FUZZY_PREFIX, DEFAULT_MIN_FUZZY_LENGTH, DEFAULT_UNICODE_AWARE);
    }

    public XNRTSuggester(Analyzer indexAnalyzer, Automaton queryPrefix, Analyzer queryAnalyzer, int options, int maxSurfaceFormsPerAnalyzedForm, int maxGraphExpansions,
                         boolean preservePositionIncrements, FST<Pair<Long, BytesRef>> fst, boolean hasPayloads, int maxAnalyzedPathsForOneInput,
                         int sepLabel, int payloadSep, int endByte, int holeCharacter, int maxEdits, boolean transpositions, int nonFuzzyPrefix, int minFuzzyLength,
    ...
    private static List<XLookupResult.XStoredField> getPayloadFields(final int docID, final Set<String> payloadFieldNames, final AtomicReader reader) throws IOException {
    ...
    protected List<FSTUtil.Path<Pair<Long, BytesRef>>> getFullPrefixPaths(List<FSTUtil.Path<Pair<Long, BytesRef>>> prefixPaths,
        ...
        if (isFuzzyEnabled()) {
            ...
            try {
                PrintWriter pw = new PrintWriter("/tmp/automata.dot");
                pw.write(levA.toDot());
                //Util.toDot(fst, pw, true, true);
                pw.close();
            } catch (IOException e) {
    ...
    static final class XPayLoadProcessor {
        ...
        /**
         * Returns a {@link org.apache.lucene.util.BytesRef} of
